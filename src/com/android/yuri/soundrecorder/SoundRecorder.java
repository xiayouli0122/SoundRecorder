...
import com.android.yuri.soundrecorder.setting.SettingActivity;
...
class RemainingTimeCalculator {
    ...
    public void setFileSizeLimit(File file, long maxBytes) {
    	System.out.println("maxBytes-->" + mMaxBytes);
        mRecordingFile = file;
        mMaxBytes = maxBytes;
    }
...
public class SoundRecorder extends Activity 
    ...
    public static ArrayList<String> audio_list = null;
    private boolean isFinish = false;
    public static final String AMR_EXTENSION = ".amr";
    public static final String THREEGPP_EXTENSION = ".3gpp";
    public static String audio_extension = "";
    
    public static final String SHARED_NAME = "record_shared";
    public static final String AUDIO_FORMAT = "audio_format";
    
    private static SharedPreferences sharedPreferences;
    
    public static String file_save_path = "";
    public static final String DEFAULT_FILE_SAVE_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/recordings";
...
//            mRequestedType = AUDIO_3GPP;
        ...
        audio_list = new ArrayList<String>();
    ...
    protected void onResume() {
    	...
    	//get file save path from sharedPreferences
    	file_save_path = sharedPreferences.getString(FILE_PATH, DEFAULT_FILE_SAVE_PATH);
    	int ext = sharedPreferences.getInt(AUDIO_FORMAT, 0);
        if (0 == ext) {
			audio_extension = AMR_EXTENSION;
		}else {
			audio_extension = THREEGPP_EXTENSION;
		}
    	super.onResume();
    }
    ...
    public void onClick(View button) {
        ...
        switch (button.getId()) {
            case R.id.recordButton:
            	if (mRecorder.mState == Recorder.IDLE_STATE) {
            		if(DEBUG) Log.d(TAG, "press record button --> start recording");
            		mRecorder.setSampleLength();
            		audio_list.clear();
                    ...
                    if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                        ...
                        if (AUDIO_AMR.equals(mRequestedType)) {
                            ...
                            mRecorder.startRecording(MediaRecorder.OutputFormat.AMR_NB, audio_extension, this);
                        } 
				...
				}else if (mRecorder.mState == Recorder.RECORDING_STATE) {
					if(DEBUG) Log.d(TAG, "press record button --> pause recording");
					isPause = true;
					audio_list.add(mRecorder.mSampleFile.getPath());
					mRecorder.pause();
				}else if (mRecorder.mState == Recorder.PAUSE_STATE) {
					if(DEBUG) Log.d(TAG, "press record button --> resume recording");
					mRemainingTimeCalculator.reset();
					if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                        mSampleInterrupted = true;
                        mErrorUiMessage = getResources().getString(R.string.insert_sd_card);
            ...
            case R.id.stopButton:
            	if(DEBUG) Log.d(TAG, "press stop button");
//                mRecorder.stop();
            	if (isPause) {
					if (mRecorder.mState == Recorder.PAUSE_STATE) {
						mRecorder.mergeRecordFiles(audio_list,audio_extension);
					}else if (mRecorder.mState == Recorder.RECORDING_STATE) {
						audio_list.add(mRecorder.mSampleFile.getPath());
						mRecorder.mergeRecordFiles(audio_list,audio_extension);
					}
            ...
            case R.id.acceptButton:
                ...
                mStateMessage2.setText(R.string.press_record);
                ...
                String timeStr = String.format(mTimerFormat, 0, 0);
...
//                    saveSample();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_setting:
			...
			intent.setClass(SoundRecorder.this, SettingActivity.class);
			startActivity(intent);
			break;
		case R.id.menu_list:
			Intent intent2 = new Intent();
			intent2.setClass(SoundRecorder.this, RecorderList.class);
    ...
    private void saveSample() {
        ...
        Toast.makeText(SoundRecorder.this, this.getResources().getString(R.string.msg_saved)
        		+ " " + mRecorder.sampleFile().getAbsolutePath(), Toast.LENGTH_LONG).show();
    }
...
//                mRecordButton.setFocusable(false);
            ...
            case Recorder.PAUSE_STATE:
                ...
                mStateLED.setImageResource(R.drawable.idle_led);
                mStateMessage2.setVisibility(View.VISIBLE);
                mStateMessage2.setText(res.getString(R.string.pause_recording));
                
                mStateProgressBar.setVisibility(View.INVISIBLE);
                mExitButtons.setVisibility(View.INVISIBLE);
                
                setTitle(res.getString(R.string.pause_recording));
            	break;
